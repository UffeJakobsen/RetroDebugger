                              ...
                              CLOCK rclk)
{
    ...
    if (value) {
        cia_context->c64d_irq_flag = 1; // c64d
    }
}
...
void cia_update_ta(cia_context_t *cia_context, CLOCK rclk) // c64d: non-static for ViceWrapper
{
    CLOCK tmp, last_tmp;
...
void cia_update_tb(cia_context_t *cia_context, CLOCK rclk) // c64d: non-static for ViceWrapper
{
    CLOCK tmp, last_tmp;
 ...
 * UBYTE        TODL_MIN
 * UBYTE        TODL_HR
 * uint32_t        TOD_TICKS       clk ticks till next tenth of second
 *
 * UBYTE        IRQ             0=IRQ line inactive, 1=IRQ line active
 *
 *                              These bits have been added in V1.1
 *
 * uint16_t         TA              Timer A state bits (see ciatimer.h)
 * uint16_t         TB              Timer B state bits (see ciatimer.h)
 *
 *                              These bits have been added in V1.2
 *
 * uint8_t         SHIFTER         actual shift register (original data)
 * uint8_t         SDR_VALID       data in SDR valid to be shifted
 *
 * uint8_t         irq_enabled     IRQ enabled
 *
 * uint8_t         todtickcounter  TOD tick counter
 *
 *                              These bits have been added in V2.3
 *
 * uint8_t         SHIFTER_HI      high byte of SHIFTER
 * uint8_t         SDR_ALARM       if an sdr_alarm is pending and when
 * uint8_t         SP_CNT_IN       input state of SP and CNT
 *
 *                              These bits have been added in V2.4
 *
 * uint32_t        SDR_DELAY       event/history bits for the serial data register
 * uint8_t         SP_CNT_OUT      output state of SP and CNT
 *
 *                              These bits have been added in V2.5
 *
 * uint32_t        IFR_DELAY       event/history bits for the interrupt flag reg.
 * uint8_t         ACK_IRQFLAGS    flags in IFR to acknowledge, i.e. clear.
 * uint8_t         NEW_IRQFLAGS    new flags in IFR that were set recently.
 */
