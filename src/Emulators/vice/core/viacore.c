...
// c64d
uint8_t c64d_viacore_peek(via_context_t *via_context, uint16_t addr)
{
    uint8_t value;
    CLOCK rclk = *(via_context->clk_ptr);
    ...
    switch (addr)
    {
        case VIA_PRA:
        case VIA_PRA_NHS:
            value = ((via_context->via[VIA_PRA] & via_context->via[VIA_DDRA])
        ...
        case VIA_T1CL:
        {
            CLOCK val = via_context->t1zero - rclk - 2;
        ...
        case VIA_T2CL:
        {
            CLOCK val = via_context->t2zero - rclk;
            return (uint8_t)(val & 0xff);
        }
