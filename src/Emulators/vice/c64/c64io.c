...
static inline uint8_t io_read(io_source_list_t *list, uint16_t addr)
{
    ...
    /* only one valid I/O source was read, return its value */
...
static void io_source_ioreg_add_onelist(struct mem_ioreg_list_s **mem_ioreg_list, io_source_list_t *current)
{
    ...
    while (current) {
        ...
        mon_ioreg_add_list(mem_ioreg_list, current->device->name, current->device->start_address, end, current->device->dump, NULL, current->device->mirror_mode);
        current = current->next;
    }
}
