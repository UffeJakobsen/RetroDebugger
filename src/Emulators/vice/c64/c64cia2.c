...
{
    uint8_t val = ciacore_read(machine_context.cia2, addr);
...
{
	return c64d_ciacore_peek(machine_context.cia1, addr);
}

uint8_t c64d_cia2_peek(uint16_t addr)
{
	return c64d_ciacore_peek(machine_context.cia2, addr);
}
