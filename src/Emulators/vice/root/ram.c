...
static RAMINITPARAM mainramparam = {
    0,      /* start_value - RAMInitStartValue */
    64,     /* value_invert - RAMInitValueInvert */
    0,      /* value_offset - RAMInitValueOffset */

    0,      /* pattern_invert - RAMInitPatternInvert */
    0,      /* pattern_invert_value - RAMInitPatternInvertValue */

    0,      /* random_start - RAMInitStartRandom */
    0,      /* random_repeat - RAMInitRepeatRandom */

    0,      /* random_chance - RAMInitRandomChance */
};
...
static resource_int_t resources_int[] = {
    { "RAMInitValueOffset", 0, RES_EVENT_SAME, NULL,
    ...
    { "RAMInitPatternInvertValue", 0, RES_EVENT_SAME, NULL,
...
static unsigned int random_method_geom_next(double log_1mp)
{
    ...
    return (g > (double)UINT_MAX) ? UINT_MAX : (unsigned int)g;
}
...
void ram_init_with_pattern(uint8_t *memram, unsigned int ramsize, RAMINITPARAM *ramparam)
{
    ...
    for (offset = 0; offset < ramsize; offset++) {
        ...
        if (ramparam->random_start && ramparam->random_repeat) {
            k = ((offset % ramparam->random_repeat) < (unsigned int)ramparam->random_start) ? lib_unsigned_rand(0, 0xff) : 0;
        }
