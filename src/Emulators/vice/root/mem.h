...
/*
 * uint32_t addr allows injection on machines with more than 64Kb of RAM.
 * Injection should be made to follow (mostly) how load would write to
 * RAM on that machine.
 */
void mem_inject(uint32_t addr, uint8_t value);

read_func_t rom_read, rom_trap_read, zero_read;
store_func_t rom_store, rom_trap_store, zero_store;

read_func_t mem_read;
store_func_t mem_store;

/* ------------------------------------------------------------------------- */

/* VICE 3.10 bank array flags (used by monitor.c) */
