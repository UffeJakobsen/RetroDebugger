...
static event_list_state_t *network_create_event_list(uint8_t *remote_event_buffer)
{
    ...
    DBGT(("network_create_event_list entry: %p", (void *)bufptr));
    ...
    if (bufptr == NULL) {
        ...
        do {
            DBGT(("network_create_event_list: %p", (void *)bufptr));
    ...
    DBGT(("network_create_event_list exit: %p", (void *)bufptr));
...
static void network_test_delay(void)
{
    ...
    if (network_mode == NETWORK_SERVER_CONNECTED) {
        ...
        for (i = 0; i < NUM_OF_TESTPACKETS - 1; i++) {
            ...
            DBG(("packet_delay[%d]=%ld", i, packet_delay[i]));
        }
        DBG(("vsyncarch_frequency = %ld", vsyncarch_frequency()));
...
int network_start_server(void)
{
    ...
    do {
        ...
        if (!listen_socket) {
            DBG(("vice_network_server failed, can not start server."));
            break;
        }
