...
/* Local IEEE device enable state (replaces iec-ieee488-shared dependency) */
static int parallel_ieee_device_enabled[IECBUS_NUM];

void parallel_bus_enable(unsigned int unit, unsigned int enable)
{
    int n;
    DBG(("parallel_bus_enable unit: %u enable: %u", unit, enable));
    parallel_ieee_device_enabled[unit] = enable ? 1 : 0;
    parallel_emu = 0;
    for (n = 0; n < IECBUS_NUM; n++) {
        parallel_emu |= parallel_ieee_device_enabled[n];
    }
    DBG(("parallel_bus_enable parallel_emu: %d", parallel_emu));
}

/* Called from resource setter for IEEE device enable */
...
{
    /* IEEE-488 device resources are not needed for C64 IEC bus.
       When PET/CBM2/VIC20 IEEE-488 cartridge support is added,
       register IEEEDevice resources here. */
    return 0;
}

int parallel_cmdline_options_init(void)
{
    /* IEEE-488 command line options are not needed for C64 IEC bus. */
    return 0;
}
