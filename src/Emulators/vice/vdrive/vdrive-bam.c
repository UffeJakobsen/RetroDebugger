...
/* c64d: removed static — called externally by c1541.c */
int vdrive_bam_isset(vdrive_t *vdrive, uint8_t *bamp, unsigned int sector)
{
    uint8_t *t = &(bamp[1 + (sector >> 3)]);
    int p = (int)((t - vdrive->bam) >> 8);
...
/* c64d: restored from VICE 3.1 — removed in 3.10 but still called by
   drive-resources.c and drive-snapshot.c */
