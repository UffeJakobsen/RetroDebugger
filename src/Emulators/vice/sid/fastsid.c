...
static int16_t fastsid_calculate_single_sample(sound_t *psid, int i)
{
	...
	if ((psid->voiceMask & 0x01) == 0x00)
	{
		o0 = 0;
	}

	if ((psid->voiceMask & 0x02) == 0x00)
	{
		o1 = 0;
	}

	if ((psid->voiceMask & 0x04) == 0x00)
	{
		o2 = 0;
	}

	returnValue = (int16_t)(((int32_t)((o0 + o1 + o2) >> 20) - 0x600) * psid->vol);
...
static void fastsid_set_voice_mask(sound_t *psid, uint8_t voiceMask)
{
	psid->voiceMask = voiceMask;
}
