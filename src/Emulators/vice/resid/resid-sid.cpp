...
SID::SID()
{
  ...
  voice_gain[1] = 1.0f;
  voice_gain[2] = 1.0f;
  voice_gain_fixed[0] = 256;
  voice_gain_fixed[1] = 256;
  voice_gain_fixed[2] = 256;
  voice_gain_active = false;

  voice_sample_callback = 0;
  voice_sample_callback_user_data = 0;
}

// ----------------------------------------------------------------------------
// Update fixed-point gain values and active flag from float voice_gain[].
// Call this after modifying voice_gain[] (e.g. from GT2 mixer).
// ----------------------------------------------------------------------------
void SID::update_voice_gain()
{
  voice_gain_fixed[0] = (int)(voice_gain[0] * 256.0f);
  voice_gain_fixed[1] = (int)(voice_gain[1] * 256.0f);
  voice_gain_fixed[2] = (int)(voice_gain[2] * 256.0f);
  voice_gain_active = (voice_gain_fixed[0] != 256
                    || voice_gain_fixed[1] != 256
                    || voice_gain_fixed[2] != 256);
}

void SID::set_voice_sample_callback(VoiceSampleCallback callback, void *userData)
{
  voice_sample_callback = callback;
  voice_sample_callback_user_data = userData;
}

RESID_INLINE void SID::emit_voice_sample(short mix)
{
  if (unlikely(voice_sample_callback)) {
    voice_sample_callback(voice_sample_callback_user_data,
                          voice[0].output(), voice[1].output(), voice[2].output(), mix);
  }
}
...
// c64d: chip number for channel routing
...
void SID::clock(cycle_count delta_t)
{
  ...
  if (unlikely(voice_gain_active)) {
    filter.clock(delta_t,
                 (voice[0].output() * voice_gain_fixed[0]) >> 8,
...
int SID::clock_fast(cycle_count& delta_t, short* buf, int n, int interleave)
{
  ...
  for (s = 0; s < n; s++) {
    ...
    short mixed_sample = amplify(output(), scaleFactor);
    buf[s*interleave] = mixed_sample;
    emit_voice_sample(mixed_sample);
  }
...
int SID::clock_resample(cycle_count& delta_t, short* buf, int n, int interleave)
{
  ...
  for (s = 0; s < n; s++) {
    ...
    short mixed_sample = amplify(v, scaleFactor);
    buf[s*interleave] = mixed_sample;
    emit_voice_sample(mixed_sample);
  }
