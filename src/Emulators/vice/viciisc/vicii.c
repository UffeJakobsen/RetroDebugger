...
void vicii_raster_draw_handler(void)
{
    ...
    if (vicii.raster.current_line == 0) {
        ...
        if ((unsigned int)vicii.last_displayed_line < vicii.screen_height) {
            vsync_do_vsync(vicii.raster.canvas, 0, 0);
        }
    }
...
void c64d_vicii_render_when_paused()
{
	raster_draw_buffer_ptr_update(&vicii.raster);
}
...
int vicii_dump(void)
{
    ...
    /* VICE 3.10: fix column width display (CSEL=0 gives 38 columns, CSEL=1 gives 40) */
...
{
	*cD021 = vicii.regs[0x21] & 0x0F;
	*cD022 = vicii.regs[0x22] & 0x0F;
	*cD023 = vicii.regs[0x23] & 0x0F;
	*cD025 = vicii.regs[0x25] & 0x0F;
	*cD026 = vicii.regs[0x26] & 0x0F;
	*cD027 = vicii.regs[0x27] & 0x0F;
	*cD800 = c64d_colorram_peek(0xD800) & 0x0F;
}
