...
/* ACTION_NONE: UI action constant from VICE 3.10 uiactions.h (stub for embedded build) */
#ifndef ACTION_NONE
...
static void joystick_process_latch(void)
{
    if (c64d_input_latch_immediate) {
        /* c64d: apply immediately for deterministic recording/replay */
        alarm_unset(joystick_alarm);
        alarm_context_update_next_pending(joystick_alarm->context);
...
{
    /* c64d: guard against NULL or invalid factory value for joymap */
    if (val == NULL) {
        return 0;
    }
...
int joystick_resources_init(void)
{
    ...
    /* c64d: use empty default path — joymap files are not used in our embedded build.
     * The original call archdep_default_joymap_file_name() had no visible prototype here,
     * causing the ARM64 compiler to treat the return as int (32-bit), truncating the
     * 64-bit heap pointer and crashing in resources_set_defaults() later. */
    joymap_factory = lib_strdup("");
...
static int joystick_snapshot_read_module(snapshot_t *s, int port)
{
    ...
    /* Sync latch with restored joystick value so that if the joystick alarm
       fires, it re-applies the correct snapshot state instead of stale data
       from a previous input event replay. Also clear joypad_status to prevent
       stale button tracking from corrupting the next joystick event. */
    latch_joystick_value.values[port + 1] = joystick_value[port + 1];
    memset(joypad_status, 0, sizeof(joypad_status));
...
joystick_device_t *joystick_device_by_index(int index)
{
    if (index < 0 || index >= num_joystick_devices) {
        ...
        log_warning(joy_log,
                    ...
                    __func__, (size_t)num_joystick_devices);
        return NULL;
    }
...
void c64d_joystick_key_up(int key, unsigned int joyport)
{
    ...
    else if (key == JOYPAD_W) column = JOYSTICK_KEYSET_W;
    else if (key == JOYPAD_E) column = JOYSTICK_KEYSET_E;
    else if (key == JOYPAD_FIRE) column = JOYSTICK_KEYSET_FIRE;
